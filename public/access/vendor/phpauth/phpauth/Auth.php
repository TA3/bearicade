...
class Auth
{
    ...
    */
    ...
    public function register($email, $password, $repeatpassword, $tfaSecret, $params = Array(), $captcha = NULL, $sendmail = NULL)
    {
        $return['error'] = true;
        $block_status = $this->isBlocked();
        ...
        $addUser = $this->addUser($email, $password, $tfaSecret, $params, $sendmail);
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT tfasecret FROM {$this->config->table_users} WHERE email = ?");
        $query->execute(array($email));
        ...
        return $query->fetch(\PDO::FETCH_ASSOC)['tfasecret'];
    }
    public function getSecretFromID($id)
    {
        $query = $this->dbh->prepare("SELECT tfasecret FROM {$this->config->table_users} WHERE id = ?");
...
/**
 *PLEASE ADD A BETTER WAY OF CHECKING IP ADDRESS, IP ADDRESS COULD CHANGE DURING A SESSION
...
*/
    ...
    */
    ...
    public function isAdmin($hash)
    {
        ...
        $isAdminUID = $query->fetch(\PDO::FETCH_ASSOC)['uid'];
        $query = $this->dbh->prepare("SELECT isAdmin FROM {$this->config->table_users} WHERE id = ?");
        $query->execute(array($isAdminUID));
        if ($query->rowCount() == 0) {
            return false;
        }
        return (bool)$query->fetch(\PDO::FETCH_ASSOC)['isAdmin'];
    ...
    public function loadConfig()
    {
        ...
        foreach($query as $row){
            $configArray[$row['setting']] = $row['value'];
        }
        unset($configArray['site_key']);
        unset($configArray['bcrypt_cost']);
        return $configArray;
    ...
    */
    ...
    protected function addUser($email, $password, $tfaSecret, $params = array(), &$sendmail)
    {
        $return['error'] = true;
        $query = $this->dbh->prepare("INSERT INTO {$this->config->table_users} (isactive, tfaSecret, isAdmin) VALUES (0,'$tfaSecret',0)");

        if (!$query->execute()) {
            $return['message'] = $this->lang["system_error"] . " #03";
            //$return['message'] = $query->errorInfo();
            return $return;
        }
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, password = ?, isAdmin = 0, tfaSecret = ?, isactive = ? {$setParams} WHERE id = ?");

        $bindParams = array_values(array_merge(array($email, $password, $tfaSecret, $isactive), $params, array($uid)));
